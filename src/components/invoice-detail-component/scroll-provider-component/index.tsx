...
import { ChildProps, ScrollContextInterface } from './types';

const withinLimits = (val: number, min: number, max: number): number =>
  val > max ? max : val < min ? min : val;

const ScrollContext = React.createContext<ScrollContextInterface>({
  opacity: 0,
  maxOffset: 0,
  offset: 0,
  titleShowing: false,
...
export const AnimatedScrollView = (props: ChildProps & ScrollViewProps) => {
